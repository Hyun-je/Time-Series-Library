...
class Decoder(nn.Module):
    def __init__(self, input_size, hidden_size, dropout, seq_len, use_act):
        ...
        self.lstm_dec = nn.GRU(input_size=hidden_size, hidden_size=hidden_size, dropout=dropout, batch_first=True)
...
class Model(nn.Module):
    ...
    # def __init__(self, input_size, hidden_size, dropout_ratio, seq_len, use_act=True):
        ...
        self.encoder = Encoder(input_size=self.input_size, hidden_size=self.hidden_size, dropout=self.dropout_ratio, seq_len=self.seq_len)
        self.decoder = Decoder(input_size=self.input_size, hidden_size=self.hidden_size, dropout=self.dropout_ratio, seq_len=self.seq_len, use_act=use_act)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        x_enc, enc_out = self.encoder(x_enc)
